...
output_dir ="/media/sf_research/cass/code/output/230426polar/"
...
for image in images_path:
    ...
    imgarray = np.array(im_cropped)
    cent=(x,y)
    max_radius = 700
    imarray=cv2.linearPolar(imgarray,cent,max_radius,cv2.WARP_FILL_OUTLIERS)
    fourier_im = np.fft.fftshift(np.fft.fft2(imarray))


    imgarray[x:x+5, y:y+5]=1
    fig, axis = plt.subplots(2, 2,figsize=(20,20), dpi=80)
    axis[0][1].imshow(imarray, cmap="gray")
    axis[0][0].imshow(imgarray, cmap="gray")
    axis[0][1].set_title("Polar Image " + str(c), fontsize=12)
    axis[0][0].set_title("Original Image" + str(c), fontsize=12)
